...
- `ebrdify(data, var, var_format)` — appends classification columns: `ebrd`
  ...
  (alternative grouping), `ebrd_shareholder`.
...
- [R/lookups.R](R/lookups.R) — internal data. The master `.ebrd_economies`
  data frame and the named-vector lookups derived from it. **Built once when the
  namespace loads, not per call** — this is the main performance lever.
- [R/utils.R](R/utils.R) — internal helpers: `.clean_input()`,
  `.detect_format()`, `.to_iso3c()` (shared format detection + ISO3 conversion).
- [R/ebrdify.R](R/ebrdify.R) — `ebrdify()`.
- [R/list_ebrd.R](R/list_ebrd.R) — `list_ebrd()`.
- [R/canonise.R](R/canonise.R) — `canonise()`.

`ebrdify()` and `canonise()` both route identifier → ISO3 through `.to_iso3c()`,
which only calls `countrycode` on the *unique* non-ISO3 values (the expensive
step) and uses O(1) named-vector lookups for everything else.
...
- Tests live in `tests/testthat/`; `tests/testthat.R` runs `test_check("ebrdify")`.
- Bump `Version` in `DESCRIPTION` and add a `NEWS.md` entry for any
  classification/naming change. NEWS tracks the **R** implementation only.
